				...
				<div class="box-body">
					...
					<div class="form-group">
						...
						<select class="form-control select2" id="role_id" name="role_id[]" multiple="">
							...
							<option value="{{$role->id}}" {{old('role_id') && in_array($role->id, old('role_id')) ? 'selected' : ''}}>{{$role->role_name}}</option>
							@endforeach
						</select>
					</div>
					<div class="form-group">
						<label for="name">姓名</label>
						<input type="text" name="name" id="name" class="form-control" placeholder="请输入姓名" value="{{old('name')}}">
					</div>
					<div class="form-group">
						<label for="sex">性别</label>
						<select id="sex" name="sex" class="form-control">
							@foreach($sexs as $value => $text)
							<option value="{{$value}}" {{old('sex') && old('sex') == $value ? 'selected' : ''}}>{{$text}}</option>
							@endforeach
						</select>
					</div>
					<div class="form-group">
						<label for="tellphone">手机号码</label>
						<input type="text" name="tellphone" id="tellphone" class="form-control" placeholder="请输入手机号码" value="{{old('tellphone')}}">
					</div>
					<div class="form-group">
						<label for="email">邮箱</label>
						<input type="text" name="email" id="email" class="form-control" placeholder="请输入邮箱" value="{{old('email')}}">
					</div>
					<div class="form-group">
						<label for="address">地址</label>
						<input type="text" name="address" id="address" class="form-control" placeholder="请输入地址" value="{{old('address')}}">
					</div>
				</div>
